...
namespace Drupal\adimeo\config;
...
class ConfigBit {
   ...
   *   Config bit file source eg. adimeo.
   *
   * @return array|int
   *   array of config bit.
   *   FALSE if we do not have the config_bit in the config bit file.
   */
  public static function getConfigBit($config_bit_file_name, $type = 'profile', $project = 'adimeo') {
    ...
    if (file_exists($full_config_bit_file_name)) {
      ...
      throw new \Exception('Config bit file does not exist! '.$config_bit_file_name);
    }
  }
   ...
   */
  public static function doWeHaveThisConfigBit($config_bit_file_name, $type = 'profile', $project = 'adimeo') {
    // Generate full path to config file.
    $full_config_bit_file_name = drupal_get_path($type, $project) . '/' . $config_bit_file_name;
   ...
   */
  public static function getList($config_bit_file_name, $condition_name, $condition_value, $sublist = NULL, $type = 'profile', $project = 'adimeo') {
   ...
   *   Condition value for the condition name in the config bit file.
   */
  public static function actionArchiveFiles($config_bit_file_name, $condition_name, $condition_value, $type = 'profile', $project = 'adimeo') {
    $config_bit_data = ConfigBit::getConfigBit($config_bit_file_name, $type, $project);
   ...
   *   Condition value for the condition name in the config bit file.
   */
  public static function actionUnArchiveFiles($config_bit_file_name, $condition_name, $condition_value, $type = 'profile', $project = 'adimeo') {
    $config_bit_data = ConfigBit::getConfigBit($config_bit_file_name, $type, $project);
    if (isset($config_bit_data['type'])
        && $config_bit_data['type'] = 'action'
        && isset($config_bit_data['action'])
   ...
   */
  public static function actionAdd($config_bit_file_name, $condition_name, $condition_value, $target, $type = 'profile', $project = 'adimeo') {
    $config_bit_data = ConfigBit::getConfigBit($config_bit_file_name, $type, $project);
   ...
   *   Targent item in the parent config file.
   */
  public static function actionRemove($config_bit_file_name, $condition_name, $condition_value, $target, $type = 'profile', $project = 'adimeo') {
    $config_bit_data = ConfigBit::getConfigBit($config_bit_file_name, $type, $project);
